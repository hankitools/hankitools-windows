...
| Selected file recycling | `src/IgezziGuard/MaintainPanel.cs`, `DuplicatePanel.cs`, `FileInventory.cs`: `CleanupPolicy`; `RecycleService.cs`: `RecycleService`, `RecycleGuard` | Review/selection plus personal-folder/local-fixed-drive policy, changed-file/reparse checks, Shell `IFileOperation` on STA, recycle-only flag and per-item veto. No permanent-delete fallback. Restore is through Windows Recycle Bin, not `ChangeJournal`. |
