...
| cleanup-recycle-restore | In a disposable Documents subfolder, inventory, filter, multi-select and Delete…. Cancel once; recycle a fixture and restore via Recycle Bin; delete another permanently only after ticking "I understand". Verify network/removable/system/program/AppData/reparse targets are refused, and that deletions appear in System actions. |
| app-uninstall-leftover | Uninstall a disposable test app (MSI and EXE installer) through Apps → Uninstall…, decline UAC once, then complete it; confirm the list no longer shows it. Delete a test app's folder by hand, refresh, remove its leftover entry, and restore it from the .reg backup. |
