...
internal sealed class ToolLauncher : Form
{
    ...
    {
        public override string ToString() => Localizer.Route(Name);
        internal string SearchText => Localizer.Route(Name) + " " + Name + " " + Localizer.SearchKeywords(string.Join(" ", new[] {
    ...
    {
        Text = Localizer.T("Find a tool"); Size = new Size(700, 520); MinimumSize = new Size(520, 380);
        ...
        var query = new TextBox { Dock = DockStyle.Top, PlaceholderText = Localizer.T("Search tools, e.g. DNS, duplicates, pagefile…"), AccessibleName = Localizer.T("Search tools") };
        var results = new ListBox { Dock = DockStyle.Fill, IntegralHeight = false, BorderStyle = BorderStyle.None, ItemHeight = 32, AccessibleName = Localizer.T("Matching tools") };
        ...
        var open = new HankiButton { Text = Localizer.T("Open selected tool"), Dock = DockStyle.Bottom, Height = 42, Primary = true };
        void Filter() {
            ...
            hint.Text = open.Enabled ? Localizer.Format("{0} tools    ↑ ↓ Choose    Enter Open    Esc Close", results.Items.Count) : Localizer.T("No matching tools. Try a broader term, such as network.");
        }
