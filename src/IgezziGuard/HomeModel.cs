...
public static class PcGlance
{
    ...
    public static string Running(TimeSpan up) =>
        up.TotalHours < 1 ? "Less than an hour" : up.TotalDays < 1 ? Plural((int)up.TotalHours, "hour") : Plural((int)up.TotalDays, "day");
    ...
    public static IReadOnlyList<GlanceTileModel> Tiles(GlanceFacts f)
    {
        var tiles = new List<GlanceTileModel> {
            new("Overview", "Windows", f.Windows ?? "Not reported", f.Build is { } b ? "Build " + b : "", CardStatus.Info, null, "Diagnose  /  Windows Update"),
            f.Uptime is { } up
        ...
        if (f.SystemFree is { } free && f.SystemTotal is { } total && total > 0) {
            ...
            tiles.Add(new("Storage", $"Drive {f.SystemDrive}", Size(free) + " free", $"of {Size(total)} · {used}% used",
                free < 10L * 1073741824 || used >= 95 ? CardStatus.Problem : used >= 85 ? CardStatus.Review : CardStatus.Good, used, "Maintain"));
        } else tiles.Add(new("Storage", $"Drive {f.SystemDrive}", "Not reported", "", CardStatus.Unknown, null, "Maintain"));
        if (f.MemoryUsed is { } inUse && f.MemoryTotal is { } memory && memory > 0) {
            int used = Percent(inUse, memory);
            tiles.Add(new("Memory", "Memory in use", $"{used}%", $"{Size(inUse)} of {Size(memory)}", used >= 90 ? CardStatus.Review : CardStatus.Good, used, "Memory"));
        } else tiles.Add(new("Memory", "Memory in use", "Not reported", "", CardStatus.Unknown, null, "Memory"));
        tiles.Add(new("GPU", "Graphics", f.Gpu ?? "Not reported", f.GpuDriver is { } d ? "Driver " + d : "", f.Gpu is null ? CardStatus.Unknown : CardStatus.Info, null, "GPU"));
