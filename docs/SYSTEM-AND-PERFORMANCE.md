...
| GPU-113 | `Performance/GpuBus.cs`, Gaming check, GPU page | Done: PCIe lane width (a desktop card on x4 or fewer is a Fix my PC item) and Resizable BAR on/off from the memory window Windows assigned; Intel Arc without it is a Fix my PC item. |
| PERF-315 | `SystemMonitor.cs`, `MonitorModel.cs` | Done: network download rate per second; Monitor, Bottleneck Analyzer and Stutter Diagnostics name downloads and the likely program. |
| FIX-120, 121, 122 | `AppCrashes.cs`, `NetworkLink.cs`, `TimeSync.cs` | Done: Fix my PC reads repeated app crashes and hangs, wired and Wi-Fi link speed, and clock sync from Windows' own records. |
| PERF-320 | Performance overview → Tune my PC, `Performance/TunePlanner.cs`, `TunePanel.cs` | Done: asks what you want today (Gaming + Performance, Gaming + Quality, Creative work, Low power) and whether the display has G-SYNC/FreeSync, reads display, Windows, mouse, NVIDIA, processor, memory and storage settings, and shows a plan: changes Hanki makes (reviewed, in Recovery) and steps for you. Hanki also changes the Windows power mode (Balanced plan), offers full GPU clocks per listed game, and reads whether G-SYNC is on (answering the question and dropping the step when it is). Rules and sources: [TUNING.md](TUNING.md). |
